 ...
 * 
 * 1. TUTTA la business logic va in Actions (src/renderer/react/actions/AssumptionsActions.ts)
...
// Auto-generated step definitions for assumptions
...
Given('the application is loaded', async function() {
    ...
    const hasActions = await this.page.evaluate(() => {
        return window.AssumptionsActions !== undefined;
    });
    // Se false, devi creare src/renderer/react/actions/AssumptionsActions.ts
});

When('I perform an action on assumptions', async function() {
    // PATTERN CORRETTO: Usa Actions class, NON manipolare direttamente
    await this.page.evaluate(() => {
        // Istanzia Actions class
        const actions = new AssumptionsActions();
...
When('I save assumptions data', async function() {
...
Then('the assumptions state should be updated', async function() {
    // Verifica state attraverso store (read-only)
    const state = await this.page.evaluate(() => {
        const store = window.appStore;
        return store.getState().assumptionsState;
    });
    expect(state).toBeDefined();
});

Then('the UI should reflect the assumptions changes', async function() {
    // Verifica che il component React si sia aggiornato
    // (il component usa useStore hook per leggere state)
    const uiState = await this.page.locator('[data-testid="assumptions-display"]').textContent();
    expect(uiState).toContain('expected value');
});

// RICORDA:
// - Actions class: src/renderer/react/actions/AssumptionsActions.ts
// - Store state: src/renderer/js/store/app-store.js
// - Component: src/renderer/react/components/Assumptions.tsx (solo UI)
