Feature: UI Interactions
  ...
  Scenario: Modal managers handle conflicts between teams and categories modals
    Given I have both team management and category management functionality available
    When I open the team member modal
    Then the team modal should use container-specific event listeners
    And the modal should function independently
    When I close the team modal and open a category modal
    Then the category modal should use its own isolated event listeners
    And there should be no interference between modal managers
    And both modals should maintain proper VSCode-style button styling

  # VSCode-Style Sidebar Navigation

  Scenario: Navigate using VSCode-style sidebar icons
    Given I am viewing the main application interface
    When I click the Projects icon in the sidebar
    Then the Projects panel should open and become active
    And the Projects icon should be highlighted
    And the main content area should show the Projects page
    When I click the Capacity Planning icon
    Then the Capacity Planning panel should open
    And the Projects panel should close
    And the Capacity Planning icon should be highlighted

  Scenario: VSCode sidebar panel toggles correctly
    Given the Projects panel is currently open
    When I click the Projects icon again
    Then the Projects panel should close
    And no sidebar panel should be active
    And the Projects icon should not be highlighted
    When I click the Configuration icon
    Then the Configuration panel should open
    And the Configuration icon should be highlighted

  Scenario: Projects panel shows context-specific actions
    Given the Projects panel is open
    Then I should see Save and Export buttons in the panel footer
    When I click the Capacity Planning icon
    And the Capacity Planning panel opens
    Then the panel footer should not show Save and Export buttons
    When I click the Configuration icon
    And the Configuration panel opens
    Then the panel footer should not show Save and Export buttons

  Scenario: Capacity Planning submenu is auto-expanded
    Given the Capacity Planning panel is open
    Then the submenu should be automatically expanded
    And I should see "Resource Capacity Overview" option
    And I should see "Capacity Planning Timeline" option
    And there should be no close button for the submenu expansion

  Scenario: Enhanced navigation maintains VSCode sidebar state across sessions
    Given I am working in the Project Phases section
    And the Projects panel is open and expanded
    And I have made specific selections and view configurations
    When I close and reopen the application
    Then the Projects panel should be open and expanded
    And the application should return to the Project Phases section
    And my previous view configurations should be restored
    And the VSCode sidebar state should be persistent

  Scenario: Navigation within sidebar panels works correctly
    Given the Projects panel is open
    When I click on "Features Management" in the Projects submenu
    Then the Features page should become active
    And the Projects panel should remain open
    And "Features Management" should be highlighted in the submenu
    When I click on "Project Phases" in the submenu
    Then the Project Phases page should become active
    And "Project Phases" should be highlighted instead

  Scenario: Navigation handles dirty state with VSCode sidebar
    Given I am in the Features section with unsaved changes
    And the Projects panel is open
    When I attempt to click the Configuration icon
    ...
    Then changes should be saved and the Configuration panel should open
    When I cancel the navigation
    Then I should remain in the Features section with Projects panel open
  ...
  Scenario: Scrollable table wrapper uses full available height
    Given I have a large data table displayed
    When the table content exceeds the available viewport height
    Then the scrollable table wrapper should use the full available height
    And the table should activate scroll behavior only when content overflows
    And the scroll behavior should be smooth and responsive
    And autoscroll behavior should be improved to prevent unnecessary scrolling
